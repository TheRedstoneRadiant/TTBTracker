...
# Set the status as "Watching UofT Courses"
# insta_client = Client()
...
async def contact_user_instagram(username: str, message: str):
    """
...
async def contact_user_sms(phone_number: str, message: str):
    """
    TODO: Implement this method
    """
    pass
contact_methods = {"instagram": contact_user_instagram, "phone": contact_user_sms}
        ...
        for activity in activities:
            ...
            if ttbapi.check_for_free_seats(activity["courseCode"], activity["Semester"], activity["Activity"]):
                ...
                for method in user_profile:
                    contact_methods[method](user_profile[method], f"Your course {activity['courseCode']} {activity['Activity']} has a spot open! Act fast!")
...
async def setup_profile(interaction: nextcord.Interaction,
                        instagram_username: Optional[str] = SlashOption(name="instagram_username", description="The Instagram profile you want the bot to alert you on when vacancies are found"),
...
async def update_profile(interaction: nextcord.Interaction, user_id: int, instagram_username: Optional[str], cell_number: Optional[int]):
    ...
    database.update_user(user_id, profile_updates)
...
async def help(interaction: nextcord.Interaction):
    embed = nextcord.Embed(title="TTBTrackr Help", description="A bot which tracks UofT courses for you and notifies you when a spot opens up", color=0x00ff00)
    ...
    embed.add_field("Viewing tracked courses", value="To view all the courses you're tracking, use /viewtracked. This will send you a list of all the courses you're tracking", inline=False)
...
async def view_tracked(interaction: nextcord.Interaction):
    ...
    embed = nextcord.Embed(title="Tracked Courses", description="Here are all the courses you're tracking", color=nextcord.Color.blue())
    
    if len(activities) == 0:
        embed.add_field(name="No Courses Tracked", value="You're not tracking any courses! Use /track to begin tracking courses", inline=False)
        return        
    
    for activity in activities:
        embed.add_field(name=f"{activity['courseCode']} {activity['Activity']} {activity['Semester']}", value=ttbapi.get_name_from_code(activity['courseCode'], activity['Semester']), inline=False)
    await interaction.response.send_message(embed=embed)
...
async def untrack(interaction: nextcord.Interaction, course_code: str = SlashOption(name="course_code", description="The course code of the course you want to untrack. Example: CSC148H5"), activity: str = SlashOption(name="activity", description="The activity code which you want to untrack. Example: LEC0101"), session: str = SlashOption(
    ...
    database.remove_user_activity(interaction.user.id, course_code, session, activity)
